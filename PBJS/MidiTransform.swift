...
extension MidiTransform: JsParsable {
  ...
  static func makeMidiPairs(_ fn: JSValue, _ bodyData: SinglePatchTruss.BodyData, _ vals: [Any]) throws -> [(MidiMessage, Int)] {
    try fn.call(vals).map {
      if let msg = try? $0.arr(0).xform(MidiMessage.jsParsers) {
        ...
        return (.sysex(try fn(bodyData)), try $0.any(1).int())
      }
    }
  }
