...
extension MultiPatchTruss : JsBankParsable {
  ...
  public static let jsBankRules: [JsParseRule<MultiBankTruss>] = [
    .d([
      "compactMultiBank" : MultiPatchTruss.self,
      "patchCount" : Int.self,
      "fileDataCount" : Int.self,
      "compactTrussMap": [JsObj].self,
      "initFile?" : String.self,
      "createFile" : SinglePatchTruss.Core.ToMidiFn.self,
      "parseBody" : Int.self,
    ], {
      let patchTruss: MultiPatchTruss = try $0.x("compactMultiBank")
      let compactTrussMap: [(SynthPath, SinglePatchTruss)] = try $0.x("compactTrussMap")
      let compactByteCount = try compactTrussMap.first?.1.createEmptyBodyData().count ?? 0
